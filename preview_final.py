...
def resolve_var(name, ctx):
    ...
    for p in parts:
        if isinstance(val, dict):
            ...
            try:
                val = val[int(p)] if p.lstrip("-").isdigit() else None
            except:
                val = None
        else:
            val = None
...
def eval_simple(cond, ctx):
    cond = cond.strip().strip("()").strip()
    negate = cond.startswith("not ")
    check = cond[4:].strip() if negate else cond
    
    # Handle != comparison
    if " != " in check:
        parts = [p.strip() for p in check.split(" != ", 1)]
        l = resolve_var(parts[0], ctx) or parts[0].strip("'\"")
        r = resolve_var(parts[1], ctx) or parts[1].strip("'\"")
        result = str(l) != str(r)
...
def eval_condition(cond, ctx):
    ...
    if " and " in cond:
        ...
        return all(eval_simple(p, ctx) for p in parts)
    if " or " in cond:
        parts = [p.strip() for p in cond.split(" or ")]
        return any(eval_simple(p, ctx) for p in parts)
    return eval_simple(cond, ctx)
...
def process_template(template_str, ctx):
    ...
    def do_capture(m):
        ...
        content = m.group(2)
        content_proc = process_template(content, ctx)
        ctx[varname] = content_proc.strip()
        return ""
    result = re.sub(
        r"\{%-?\s*capture\s+(\w+)\s*-?%\}(.*?)\{%-?\s*endcapture\s*-?%\}",
        do_capture, result, flags=re.DOTALL
    )
    ...
    result = re.sub(
        ...
        do_for, result, flags=re.DOTALL
    )
    
    # 6. If-elsif-else blocks
    def do_if_chain(m):
        full = m.group(0)
        ...
        while remaining:
            parts = re.split(
                r"\{%-?\s*(elsif|else)\s+([^%}]+?)?\s*-?%\}",
                remaining, maxsplit=1, flags=re.DOTALL
            )
            if len(parts) == 1:
        ...
        for cond, bc in branches:
            if cond == "__else__":
                return process_template(bc, ctx)
    ...
    result = re.sub(
        ...
        do_if_chain, result, flags=re.DOTALL
    )
    ...
    result = re.sub(r"\{\{\s*(.*?)\s*\}\}", do_var, result)
