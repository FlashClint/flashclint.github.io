...
date: 2026-07-28
tags: [protocol, 科研笔记, liposome, membrane]
comments: true
---
...
## Calcein 基本信息
- calcein  = fluorescein + iminodiacetic acid (EDTA的关键部分)，所以calcein同时具有荧光和螯合作用 ([Ref.1](Calcein-containing liposome leakage assay#3eec2d))
...
### 影响calcein 发光的因素
([Ref.1](Calcein-containing liposome leakage assay#3eec2d))
...
#### 浓度
- 通常认为，calcein在浓度大于70 mM时会完全自淬灭（但不同条件下、不同文章给出的不尽相同）([Ref.1](Calcein-containing liposome leakage assay#3eec2d))
...
## Protocol
概览([Ref.1](Calcein-containing liposome leakage assay#3eec2d))：
